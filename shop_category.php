...
$all_products_query = mysqli_query($db, "SELECT * FROM products");
$count_products = mysqli_num_rows($all_products_query);

$current_category = (isset($_REQUEST['cat']) && $_REQUEST['cat'] !== '') ? trim($_REQUEST['cat']) : 'All Products';

if (strtolower($current_category) === 'all products') {
    $query = mysqli_query($db, "SELECT * FROM products ORDER BY product_id DESC");
} else {
    $name = mysqli_real_escape_string($db, $current_category);
    $query = mysqli_query($db, "SELECT * FROM products WHERE category = '$name' ORDER BY product_id DESC");
}
        ...
        <div class="flex flex-col lg:flex-row lg:gap-6">
            ...
            <div id="sidebar"
                ...
                <div
                    ...
                    <div class="space-y-2">
                        <?php
                        // All Products link highlight logic
                        $all_class = (strtolower(trim($current_category)) === 'all products')
                            ? 'bg-[#c28e5c] text-white font-semibold'
                        ...
                        while ($category = mysqli_fetch_array($category_query)) {
                            ...
                            $category_products_query = mysqli_query($db, "SELECT * FROM products WHERE category='" . mysqli_real_escape_string($db, $cat) . "'");
                            $category_counts = mysqli_num_rows($category_products_query);

                            // case-insensitive compare
                            $is_active = (strtolower(trim($current_category)) === strtolower(trim($cat)));
                            $cat_class = $is_active ? 'bg-[#d5cfc1] text-[#c28e5c] font-semibold' : 'text-gray-700 hover:text-[#c28e5c] hover:bg-gray-100';
                        ?>
                            <a href="shop_category.php?cat=<?php echo urlencode($cat); ?>"
                                ...
                                <div class="right text-[16px]">(<?php echo $category_counts; ?>)</div>
                            </a>
                        <?php } ?>
            ...
            <div class="right_content w-full lg:flex-1 p-1 md:px-6">
                <div class="flex items-center justify-between">
                    <div class="text-xl font-bold text-gray-800 flex">
                        <p><?php echo htmlspecialchars(isset($_REQUEST['cat']) ? $current_category : 'All Products'); ?></p>&nbsp;
                        <p>(<?php echo $counts; ?>)</p>
                    </div>
                    <div class="lg:hidden">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                    ...
                    ?>
                        ...
                        duration-500 border border-gray-200 group  cursor-pointer">
                            <a href="shop_products.php?pro=<?php echo $products['product_id']; ?>" class="block">
                                ...
                                <div class="product-image-container relative w-full overflow-hidden">
                                    ...
                                    <!-- Hover Button -->
                                    <div class="absolute inset-0 flex items-end justify-center p-3 opacity-0 group-hover:opacity-100 transition duration-500 ease-in-out w-full">
                                        <button class="w-full py-2 bg-white text-[#c28e5c] font-semibold rounded-lg shadow-md transform translate-y-4 group-hover:translate-y-0 transition duration-500 ease-in-out border-2 border-[#c28e5c] cursor-pointer">
                                ...
                                <div class="p-4 md:p-5">
                                    <div class="space-y-2">
                                        <h3 class="name font-semibold text-md text-gray-900 line-clamp-2">
