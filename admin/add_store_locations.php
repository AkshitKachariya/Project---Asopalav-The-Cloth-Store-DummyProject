...
if (isset($_REQUEST['submit_btn'])) {
    ...
    mysqli_query($db, "INSERT INTO store_location (location_name, location_address, location_link, location_phone, location_image) VALUES ('$name', '$address', '$link', '$phone', '$image')");
...
<head>
    ...
    <title>Add Store Location | Asopalav.in</title>
...
<body>
    ...
    <main class="flex-1 p-4 overflow-y-auto bg-gradient-to-br from-[#ae8dfc3b] via-purple-50 to-[#ae8dfc36]">
        <h2 class="text-2xl font-semibold text-[#512DA8] mb-4">Add Store Location :</h2>
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="space-y-5" onsubmit="return validateForm();">
            ...
            <div>
                ...
                <textarea id="location_address" name="location_address" rows="2" required placeholder="store address..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#512DA8]"></textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" id="location_link" name="location_link" required placeholder="Paste Google Maps src link only..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#512DA8]" />
                <p class="text-sm text-gray-500 mt-1">
                    Paste only the <strong>iframe src URL</strong> from Google Maps' "Embed a map" option (e.g. <code>https://www.google.com/maps/embed?... </code>)
                </p>
            </div>

            <div>
                <label for="location_image" class="block text-sm font-medium text-[#512DA8] mb-1">Upload Image</label>
                <input type="file" id="location_image" name="location_image" accept="image/*" required
