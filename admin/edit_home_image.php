...
if (isset($_REQUEST['edt'])) {
    ...
    $record = mysqli_query($db, "SELECT * FROM home_image WHERE image_id=$id");
...
if (isset($_REQUEST['submit_btn'])) {
    ...
    if ($image == "") {
        ...
        $image = $result['image_image'];
    }

    mysqli_query($db, "UPDATE home_image SET image_heading='$heading', image_text='$text', image_image='$image' WHERE image_id=$id");
...
<head>
    ...
    <title>Edit Home Image | Asopalav.in</title>
...
<body>
    ...
    <main class="flex-1 p-4 overflow-y-auto bg-gradient-to-br from-[#ae8dfc3b] via-purple-50 to-[#ae8dfc36]">
        ...
        <h2 class="text-2xl font-semibold text-[#512DA8] mb-4">Edit Home Image : </h2>
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="image_heading" name="image_heading" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#512DA8]" value="<?php echo $result['image_heading']; ?>" placeholder="image heading..." />
            </div>
            ...
            <div>
                ...
                <textarea id="image_text" name="image_text" rows="2" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#512DA8] placeholder=" image text...""><?php echo $result['image_text']; ?></textarea>
            </div>

            <div>
                <label for="image_image" class="block text-sm font-medium text-[#512DA8] mb-1">Upload Image</label>
                <input type="file" id="image_image" name="image_image" accept="image/*"
                    class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4
                   file:rounded-md file:border-0 file:text-sm file:font-semibold
                   file:bg-[#ede7f6] file:text-[#512DA8]
                   hover:file:bg-[#d1c4e9] cursor-pointer" /> <br>
                <img src="../images/<?php echo $result['image_image']; ?>" alt="" width="200px">
                <p>Image : (<?php echo $result['image_image']; ?>)</p>
            </div>
